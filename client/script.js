...
// CODE HERE
const helloButton = document.querySelector("#say-hello-button");
...
const sayHelloButton = () => {
  helloButton.classList.add("button-black");
};

helloButton.addEventListener("mouseover", sayHelloButton);
// PROBLEM 3
/*
...
// CODE HERE
helloButton.addEventListener("mouseout", () =>
  helloButton.classList.remove("button-black")
);
...
const repeatMyParam = (e) => {
  e.preventDefault();
  axios
    .get("http://localhost:3000/repeat/test")
    .then((response) => {
      console.log(response.data);
      const paragraph = document.querySelector("#repeat-text");
...
const queryFunc = (e) => {
  e.preventDefault();
  axios
    .get("http://localhost:3000/query-test?name=Elliott&age=27")
...
const createFood = (e) => {
  ...
  const obj = { newFood: food.value };
  //console.log(obj.newFood);
  if (obj.newFood !== "") {
    axios
      .post("http://localhost:3000/food", obj)
      .then((response) => {
        //console.log(response.data);
        let x = document.createElement("li");
        x.innerText = response.data[response.data.length - 1];
...
const obj = {};
axios.post("http://localhost:3000/food", obj).then((response) => {
  const array = response.data.filter((element) => {
    //console.log(element);
    return element !== null;
  });
  //console.log(response.data);
